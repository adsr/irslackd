...
(function() {
  ...
  let refreshUsers = async function(ircUser) {
    ...
    users.members.forEach((user) => {
      ...
      ircUser.slackToIrc.set(user.id, user.name);
      ircUser.ircToSlack.set(user.name, user.id);
  ...
  let refreshTeams = async function(ircUser) {
    ...
    let teams;
    try {
      teams = await ircUser.slackWeb.apiCall('usergroups.list', {
        include_count: false,
        include_disabled: false,
        include_users: false,
      });
      if (!teams.ok) throw teams;
    } catch (e) {
      irslackd.logError(ircUser, 'Failed usergroups.list: ' + util.inspect(e));
      return;
    }
