...
class Irslackd {
    ...
    if (!alreadyInSlackChanId) {
      ...
      if (slackChanId) {
        ...
        convo = await ircUser.slackWeb.apiCallOrThrow('conversations.create', {
          name: (ircChan.substr(0, 1) === '#') ? ircChan.substr(1) : ircChan,
          is_private: false,
        });
  ...
  async onIrcPart(ircUser, msg, alreadyInSlackChanId) {
    ...
    // Leave Slack channel
    let apiMethod = slackChan.substr(0, 1) === 'G' ? 'conversations.close' : 'conversations.leave';
  ...
  isMpdmChannel(chanOrNick) {
    return (chanOrNick.substr(0, 1) === '&') || (chanOrNick.substr(0, 6) === '#mpdm-');
  }
  isNormalChannel(chanOrNick) {
    return chanOrNick.substr(0, 1) === '#';
  }
  isChannel(chanOrNick) {
    return this.isNormalChannel(chanOrNick) || this.isMpdmChannel(chanOrNick);
  }
  extractThread(text) {
    let re = /^@thread-([^ ]+) /;
