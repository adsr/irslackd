...
class Irslackd {
  ...
  async on IrcInvite(ircUser, msg) {
    if (msg.args.length < 2) return;

    let ircNick = msg.args[0];
    let ircChan = msg.args[1];

    if (!ircUser.ircToSlack.has(ircChan) ||
        !ircUser.inChannel.get(ircChan)  ||
        !ircUser.ircToSlack.has(ircNick) ) {
        return;
    }
    ...
    await ircUser.slackWeb.apiCallOrThrow('channels.invite', { channel: slackChan, user: slackId });
  }
  ...
  async onIrcPrivmsg(ircUser, msg) {
    ...
    // Ignore messages to debug channel
    if (target === debugChannel) return;
