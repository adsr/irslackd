...
class Irslackd {
  ...
  async onSlackMemberJoinedChannel(ircUser, event) {
    if (ircUser.slackUserIds.includes(event.user)) return;
    let [ircNick, ircChan] = await ircUser.resolveSlackTarget(event);
  ...
  makeSlackHandler(method) {
    ...
    return function() {
      ...
      if (ircUser) {
        ...
        ircUser.logError('Could not find user state for rtm ' + rtm);
      }
    };
  }
...
class IrcUser {
  ...
  async resolveSlackUser(slackUser) {
    ...
    // Try users.info
    let user = await this.slackWeb.apiCallOrThrow('users.info', { user: slackUser });

    // Special case for when we have multiple user_ids
    // According to Slack support this is OK...
    if (user.user.id !== slackUser
      && this.slackUserIds.includes(user.user.id)
      && !this.slackUserIds.includes(slackUser)
    ) {
      this.slackUserIds.push(slackUser);
    }
