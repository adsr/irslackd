  ...
  async onIrcWho(ircUser, msg) {
    ...
    try {
      ...
      if (chanSearch) {
        ...
        nicksInChan.forEach((value, key) => {
          var slackId = ircUser.ircToSlack.get(key);
          if (typeof slackId === 'undefined') throw Error('Could not find slackId in ircToSlack');
          var slackInfo = ircUser.slackUsersInfo.get(slackId);
          if (typeof slackInfo === 'undefined') throw Error('Could not find slackId in slackUsersInfo');
