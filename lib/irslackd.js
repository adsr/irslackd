...
class Irslackd {
  ...
  async onIrcPrivmsg(ircUser, msg) {
    ...
    try {
      ...
      this.rememberSelfEcho(ircUser, apiMethod, apiParams);
      let chat = await ircUser.slackWeb.apiCall(apiMethod, apiParams);
  ...
  async onSlackMessage(ircUser, event) {
    // Prevent self-echo
    if (this.preventSelfEcho(ircUser, event)) return;
  ...
  rememberSelfEcho(ircUser, apiMethod, apiParams) {
    let event = {
      type: 'message',
      subtype: apiMethod === 'chat.meMessage' ? 'me_message' : null,
      text: apiParams.text,
    };
    let maxSelfEchoEntries = 1024;
    ircUser.selfEchoList.unshift(this.selfEchoKeyFromEvent(event));
    ircUser.selfEchoList = ircUser.selfEchoList.slice(0, maxSelfEchoEntries);
  }
  preventSelfEcho(ircUser, event) {
    if (event.user !== ircUser.slackUserId) return false;
    console.log('preventSelfEcho', util.inspect(ircUser.selfEchoList), util.inspect(event));
    let idx = ircUser.selfEchoList.indexOf(this.selfEchoKeyFromEvent(event));
    if (idx === -1) return false;
    ircUser.selfEchoList.splice(idx, 1);
    return true;
  }
  selfEchoKeyFromEvent(event) {
    let key = [];
    if (event.type) key.push(event.type);
    if (event.subtype) key.push(event.subtype);
    if (event.text) key.push(event.text);
    return key.join('|');
  }
