...
class Irslackd {
  ...
  async onIrcJoin(ircUser, msg, alreadyInSlackChanId) {
    ...
    ircChans.forEach(async(ircChan) => {
      self.onIrcJoinOne(ircUser, msg, alreadyInSlackChanId, ircChan);
    });
  }
  async onIrcJoinOne(ircUser, msg, alreadyInSlackChanId, ircChan) {
  ...
  async onIrcPing(ircUser, msg) {
    this.ircd.write(ircUser.socket, 'irslackd', 'PONG', []);
  }
