...
class Irslackd {
  ...
  async onIrcJoin(ircUser, msg, alreadyInSlackChanId) {
    ...
    if (!alreadyInSlackChanId) {
      ...
      try {
        convo = await ircUser.slackWeb.apiCall('channels.join', { name: slackChanName });
    ...
    try {
      convo = await ircUser.slackWeb.apiCall('conversations.info', { channel: slackChanId });
      if (!convo.ok) throw convo;
    } catch (e) {
  ...
  async onIrcPart(ircUser, msg, alreadyInSlackChanId) {
    ...
    // Leave Slack channel
    let convo;
    let apiMethod = 'conversations.leave';
    try {
      if (slackChan.substr(0, 1) === 'G') apiMethod = 'mpim.close';
      convo = await ircUser.slackWeb.apiCall(apiMethod, { channel: slackChan });
      if (!convo.ok) throw convo;
    } catch (e) {
  ...
  async onSlackMessage(ircUser, event) {
    ...
    if (event.subtype === 'me_message') {
      message = this.meText(event.text);
    } else {
      message = event.text;
    }

    // Send to IRC
    this.ircd.write(ircUser.socket, ircNick, 'PRIVMSG', [ ircTarget, ':' + message ]);
  }
