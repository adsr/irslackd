...
class Irslackd {
  ...
  async onIrcTopic(ircUser, msg) {
    ...
    ircTopic = this.decodeEntities(res.channel.topic.value);
  ...
  async onSlackMessage(ircUser, event) {
    ...
    if (event.attachments) {
      event.attachments.forEach((attachment, idx) => {
        if (attachment.fallback) messages.push('> ' + self.ircizeText(ircUser, attachment.fallback));
  ...
  async onSlackChannelJoined(ircUser, event) {
    let ircChan = await this.resolveSlackChannel(ircUser, event.channel.id);
    if (ircChan) {
      await this.onIrcJoinOne(ircUser, ircChan, event.channel.id);
    }
  }
  ...
  async onIrcAtSlackPrivmsg(ircUser, msg) {
    /*
    TODO
    const target = msg.args.shift();
    const cmdStr = msg.args.join(' ').trim();
    const [cmdArgs, cmdArgv] = this.parseAtSlackCmd(cmdStr);

    const slackTarget = ircUser.ircToSlack.get(target);
    if (!slackTarget) throw Error('No entry in ircToSlack for target ' + target)

    switch (cmdArgv[0] || '') {
      case 'history': // conversations.history
        //channel, latest, oldest, limit
      case 'search': // search.messages
        //query, count
      case 'thread': // conversations.replies or onIrcPrivmsg(..., thread_ts=...)
        //channel, ts, limit
      case 'react': // reactions.add
        //name, channel, timestamp
      case 'unreact': // reactions.add
        //name, channel, timestamp
    }
    */
  }
