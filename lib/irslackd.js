...
const DISPLAY_SERVER = 'api.slack.com';

class Irslackd {
  constructor(config) {
    this.onSlackReadyRunning = false;
    this.config = config;
    this.socket = null;
    this.ircd = null;
    this.rtmMap = new Map();
  ...
  listen() {
    ...
    new Map([
      ...
      ['KICK', self.makeIrcHandler(self.onIrcKick)],
      ['WHOIS', self.makeIrcHandler(self.onIrcWhois)],
      ['PING', self.makeIrcHandler(self.onIrcPing)],
      ['TOPIC', self.makeIrcHandler(self.onIrcTopic)],
  ...
  async onIrcUser(ircUser, msg) {
    ...
    new Map([
      ...
      ['group_archive', self.makeSlackHandler(self.onSlackChannelDeleted)],
  ...
  async onIrcJoinOne(ircUser, ircChan, alreadyInSlackChanId) {
    ...
    if (!alreadyInSlackChanId) {
      ...
      convo = await ircUser.slackWeb.apiCallOrThrow('channels.join', {
        name: slackChanName,
      });
  ...
  async onIrcPart(ircUser, msg, alreadyInSlackChanId) {
    ...
    try {
      ...
      } else if (typeof JSON.parse(e.message) !== 'undefined' && JSON.parse(e.message).error.data.error === 'is_archived') {
        // no worries, we don't need to leave this conversation, we where archived out of it
      } else {
        throw e;
      }
    }
  ...
  async onIrcPrivmsg(ircUser, msg) {
    ...
    if (target.substr(0, 1) === '#') {
      ...
      if (!slackChan) {
        let im = await ircUser.slackWeb.apiCallOrThrow('im.open', {
          user: slackTarget,
        });
  ...
  async onIrcMode(ircUser, msg) {
    let target = msg.args[0];
    if (msg.args.length >= 2 && target.substr(0, 1) === '#') {
  ...
  async onIrcTopic(ircUser, msg) {
    const self = this;
    var ircCode = '331';
    var ircMessage = ':No topic is set';
    try {
      if (msg.args.length > 2) {
        throw Error('/TOPIC only supports two parameters');
      }
      var channelId = ircUser.ircToSlack.get(msg.args[0]);
      if (!channelId) throw Error('IRC channel not found in irslackd cache');
      if (msg.args[1]) {
        ircCode = '332';
        ircMessage = ':' + msg.args[1];
      }
      await ircUser.slackWeb.paginateCallOrThrow('conversations.setTopic', 'topic', {
        channel: channelId,
        topic: msg.args[1],
      });
      self.ircd.write(ircUser.socket, 'irslackd', ircCode, [
        ircUser.ircNick,
        msg.args[0],
        ircMessage,
      ]);
    } catch (e) {
      var message = ':/TOPIC failed->';
      if (this.isJson(e.message)) {
        var errorFromData = typeof JSON.parse(e.message).error.data !== 'undefined' ? JSON.parse(e.message).error.data.error : 'no data element in error response';
        message +=
        JSON.parse(e.message).error.code + '->' +
  ...
  async onIrcKick(ircUser, msg) {
    ...
    try {
      ...
      if (typeof msg.args[0] !== 'undefined') {
        ircKickChannel = msg.args[0];
      }
      if (typeof msg.args[1] !== 'undefined') {
        ircKickUser = msg.args[1];
      }
      if (typeof msg.args[2] !== 'undefined') {
        ircKickMessage = msg.args[2];
      }
      // make sure user to kick is in channel
      var nicksInChan = ircUser.channelNicks.get(ircKickChannel);
      if (typeof nicksInChan === 'undefined') {
        throw new Error('Unknown channel ' + ircKickChannel);
      } else {
        nicksInChan.forEach((value, key) => {
          if (key === ircKickUser) {
            var slackId = ircUser.ircToSlack.get(key);
            if (typeof slackId === 'undefined') {
              throw new Error('User, ' + ircKickUser + ', is not in channel ' + ircKickChannel);
            }
          }
        });
      }
      var slackKickChannel = ircUser.ircToSlack.get(ircKickChannel);
      if (typeof slackKickChannel === 'undefined') throw Error('Could not find slack channel in ircToSlack');
      var slackKickUser = ircUser.ircToSlack.get(ircKickUser);
      if (typeof slackKickUser === 'undefined') throw Error('Could not find slack user in ircToSlack');
      console.log('kick channel and user: ' + slackKickChannel + '/' + slackKickUser);
      await ircUser.slackWeb.paginateCallOrThrow('conversations.kick', 'ok', {
        channel: slackKickChannel,
        user: slackKickUser,
      });
      this.ircd.write(ircUser.socket, ircUser.ircNick, 'KICK', [ircKickChannel, ircKickUser, ircKickMessage]);
    } catch (e) {
      var message;
      if (this.isJson(e.message)) {
        var errorFromData = typeof JSON.parse(e.message).error.data !== 'undefined' ? JSON.parse(e.message).error.data.error : 'no data element in error response';
        ...
        message = ':/KICK failed->' + e.message;
      }
      self.ircd.write(ircUser.socket, 'irslackd', '441', [
        ircUser.ircNick,
        ircKickUser,
        ircKickChannel,
        message,
  ...
  async onIrcWho(ircUser, msg) {
    ...
    try {
      ...
      if (chanSearch) {
        let nicksInChan = ircUser.channelNicks.get(chanSearch);
        if (typeof nicksInChan === 'undefined') {
          throw new Error('Unknown channel ' + chanSearch);
        }
        let nicksInChanProcessed = nicksInChan.size;
        for (let entry of nicksInChan) {
          let ircNickId = entry[0];
          let slackId = ircUser.ircToSlack.get(ircNickId);
          if (typeof slackId === 'undefined') throw Error('Could not find ircNickId, ' + ircNickId + ', in ircToSlack');
          let slackInfo = await ircUser.mapSlackUser(ircNickId, slackId);
          if (typeof slackInfo === 'undefined') throw Error('Could not find slackId,' + slackId + ', in slackUsersInfo');
        ...
        let ircToSlackProcessed = ircUser.ircToSlack.size;
        for (let entry of ircUser.ircToSlack) {
          let ircNickId = entry[0];
          let slackId = entry[1];
          let writeit = false;
          let slackInfo;
  ...
  async onSlackReady(ircUser, event) {
    if (this.onSlackReadyRunning) {
      console.log('onSlackReady is already running ... not running again');
      return;
    }
    this.onSlackReadyRunning = true;
    console.log('onSlackReady beginning --- (refreshing users, teams, channels)');
    // Send MOTD
    ...
    console.log('onSlackReady ended --- (refreshing users, teams, channels)');
    this.onSlackReadyRunning = false;
  }
  async onSlackMessage(ircUser, event) {
    ...
    // don't try to rejoin the channel (group_archive) otherwise it will 'create' a new (non private) one in that case
    // If not already in channel, join it
    if (event.subtype !== 'group_archive' && event.subtype !== 'channel_archive' && ircTarget.substr(0, 1) === '#' && !ircUser.isInChannel(ircTarget)) {
  ...
  async onSlackChannelJoined(ircUser, event) {
    // may have a handle to a channel json object
    let eventChannel = event.channel;
    if (typeof eventChannel === 'object') {
      eventChannel = event.channel.id;
    }

    let ircChan = await this.resolveSlackChannel(ircUser, eventChannel);
    if (ircChan) {
      await this.onIrcJoinOne(ircUser, ircChan, eventChannel);
    }
  }
  async onSlackChannelLeft(ircUser, event) {
    let ircChan = await this.resolveSlackChannel(ircUser, event.channel);
    if (ircChan) {
      await this.onIrcPart(ircUser, {
        args: [ircChan],
      }, event.channel);
    }
  }
  async onSlackChannelDeleted(ircUser, event) {
    let oldIrcChan = await this.resolveSlackChannel(ircUser, event.channel);
  ...
  async onIrcWhois(ircUser, msg) {
    ...
    try {
      // ignore channels
      if (ircNick.substr(0, 1) === '#') throw Error('Unknown user');
      let slackUser = ircUser.ircToSlack.get(ircNick);
      if (!slackUser) throw Error('Unknown user');
      let user = await ircUser.slackWeb.apiCallOrThrow('users.info', {
        user: slackUser,
      });
      // RPL_WHOISUSER
      ...
      this.ircd.write(ircUser.socket, 'irslackd', '319', [
        ircUser.ircNick,
        ircNick,
        ':' + ircUser.getChannelsForNick(ircNick).join(' '),
      ]);
      // RPL_WHOISSERVER
      // 	<source> 312 <target> <nick> <server> :<server info>
      this.ircd.write(ircUser.socket, 'irslackd', '312', [
        ircUser.ircNick,
        ircNick,
        DISPLAY_SERVER,
        ':The SLACK API',
      ]);
      if (!ircUser.ircNickActive.has(ircNick)) {
        // RPL_AWAY
        // <source> 301 <target> <nick> :<away message>
        this.ircd.write(ircUser.socket, 'irslackd', '301', [
          ircUser.ircNick,
          ircNick,
          ':Gone (Away)',
        ]);
      ...
      this.ircd.write(ircUser.socket, 'irslackd', '318', [
        ircUser.ircNick,
        ircNick,
        ircNick,
        'irslackd',
  ...
  onIrcDebugPrivmsg(ircUser, msg) {
    ...
    switch (cmd) {
      case 'dump_server':
        out = util.inspect(this, iOpts);
        break;
      case 'dump_user':
        out = util.inspect(ircUser, iOpts);
        break;
      case 'dump_rtm':
        out = util.inspect(ircUser.slackRtm, iOpts);
        break;
      case 'dump_web':
        out = util.inspect(ircUser.slackWeb, iOpts);
        break;
    }
  ...
  async resolveSlackChannel(ircUser, slackChan) {
    ...
    let convo = await ircUser.slackWeb.apiCallOrThrow('conversations.info', {
      channel: slackChan,
    });
  ...
  async resolveSlackBot(ircUser, slackBotId) {
    ...
    // Try bots.info
    let bot = await ircUser.slackWeb.apiCallOrThrow('bots.info', {
      bot: slackBotId,
    });
...
class IrcUser {
  ...
  /**
   * @returns channelsArray[]
   * @param {*} ircNick
   */
  getChannelsForNick(ircNick) {
    var channelsArray = [];
    this.channelNicks.forEach((nickMap, ircChan, theMap) => {
      if (nickMap.get(ircNick)) {
        channelsArray.push(ircChan);
      }
    });
