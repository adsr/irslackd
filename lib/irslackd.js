...
class Irslackd {
  ...
  async onIrcUser(ircUser, msg) {
    ...
    ircUser.slackUserIds = [ ircUser.slackUserId ];
  ...
  async onSlackReaction(ircUser, event, verb) {
    ...
    if (!event.item_user) {
      ...
      if (history.messages.length > 0
        ...
        || history.messages[0].latest_reply === event.item.ts)
    ...
    let ircChan    = this.resolveSlackChannel(ircUser, event.item.channel, ircReacter);
    try {
  ...
  async resolveSlackChannel(ircUser, slackChan, ircSender) {
    // Check cache
    ...
    if (convo.channel.is_im) {
      if (await ircSender === await ircUser.ircNick) {
  ...
  async resolveSlackUser(ircUser, slackUser) {
    ...
    if (user.user.id !== slackUser
      && ircUser.slackUserIds.includes(user.user.id)
      && !ircUser.slackUserIds.includes(slackUser)
    ) {
      ircUser.slackUserIds.push(slackUser);
    }
  ...
  maybeSendIrcDebug(ircUser, prefix, debugStr) {
    ...
    // Annotate ids
    debugStr = this.annotateIds(ircUser, debugStr);
  ...
  async preventSelfEcho(ircUser, event, message) {
    ...
    if (!ircUser.slackUserIds.includes(event.user)) {
  ...
  async apiCallOrThrow(method, params) {
    ...
    e = this.irslackd.annotateIds(this.ircUser, util.inspect(params));
