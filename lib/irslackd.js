...
class Irslackd {
  ...
  async onIrcAway(ircUser, msg) {
      ...
      try {
          ...
          this.ircd.write(ircUser.socket, 'irslackd', code, [ '=', reply ]);
      } catch(e) {
  ...
  async onIrcInvite(ircUser, msg) {
      ...
      if (msg.args.length < 2) {
          ...
          let ircChan = msg.args[1];

          if (!ircUser.ircToSlack.has(ircChan) || !ircUser.inChannel.get(ircChan)) {
              code = '442'; // ERR_NOTONCHANNEL
              reply = ircChan + ':You\'re not on that channel';
          }
          else if (!ircUser.ircToSlack.has(ircNick)) {
              code = '401'; // ERR_NOSUCHNICK
              reply = ircNick + ' :No such nick/channel';
          }
          else {
              code = '341'; // RPL_INVITING
              reply = ircChan +' '+ ircNick;

              let slackId = ircUser.ircToSlack.get(ircNick);
              let slackChan = ircUser.ircToSlack.get(ircChan);

              let invite;
              try {
                  invite = await ircUser.slackWeb.apiCall('channels.invite', { channel: slackChan, user: slackId });
                  if (!invite.ok) throw invite;
              } catch(e) {
                  this.logError(ircUser, 'Failed channels.invite: ' + util.inspect(e));
                  return;
              }
          }
      }
  ...
  async resolveSlackUser(ircUser, slackUser) {
    ...
    // Set cache; return
    ircNick = (user.user.profile.display_name) ? user.user.profile.display_name : user.user.name;
